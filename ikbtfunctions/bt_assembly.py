...
def make_leaves(leaf_debug=False, solver_debug=False):
    ...
    #  SUCCESS iff the arm has NO triple, which is exactly what the hybrid wants,
    #  so it goes straight into the branch with no Inverter.  It FAILs both for
    #  an arm that HAS a triple and for a table it could not read -- neither is
    #  "this arm needs simplifying".  The report's geometry statement is
    ...
    noPieperID = no_pieper_id()
    noPieperID.BHdebug = leaf_debug
    n['noPieperID'] = noPieperID

    ###  Rank the DH changes that would give the arm a Pieper triple.
    #  Reads pieper_ok and refuses to act if the analysis was unreliable.  That
    #  is now defence in depth:  no_pieper_id already FAILs on a table it could
    #  not read, so the branch should not have opened at all.
...
def build_default_bt(leaf_debug=False, solver_debug=False, nodes=None,
    ...
    #  which is what no_pieper_id answers directly -- SUCCESS when no triple of
    #  consecutive joint axes intersects or is parallel.  This used to be
    #  b3.Inverter(pieper_id), the only Inverter in the tree;  the node's
    #  polarity was swapped instead so the branch reads as what it gates on.
    #
    ...
    #  completely -- so gating the symbolic branch on the triple would stop the
    ...
    hybridBranch = b3.Sequence([nodes['noPieperID'],
                                nodes['simplifiedArm'],
